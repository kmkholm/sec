...
DEFAULT_DEEPSEEK_API_KEY = "sk-7b359324541045a897788afab0f774b9"  # User provided API key
...
class ReconTool(tk.Tk):
    ...
    def check_dependencies(self):
        ...
        # Emphasize the importance of the requests library for DeepSeek
        if not REQUESTS_AVAILABLE:
            messages.append("Requests library is required for DeepSeek AI functionality.")
    ...
    def create_menu(self):
        ...
        tools_menu.add_command(label="DeepSeek AI", command=lambda: self.notebook.select(3))  # Add DeepSeek menu item
        ...
        help_menu.add_command(label="DeepSeek Guide", command=self.show_deepseek_guide)  # Add DeepSeek guide
    ...
    def create_main_frame(self):
        ...
        self.deepseek_tab = DeepSeekTab(self.notebook, self)  # Add DeepSeek tab
        ...
        self.notebook.add(self.deepseek_tab, text="DeepSeek AI")  # Add DeepSeek tab
    ...
    def save_all_results(self):
        ...
        try:
            with open(filepath, "w", encoding='utf-8') as f:
                ...
                # Get results from DeepSeek
                deepseek_results = self.deepseek_tab.get_results()
                if deepseek_results:
                    f.write("=== DEEPSEEK AI ANALYSIS ===\n")
                    f.write(deepseek_results + "\n\n")
    ...
    def show_api_config(self):
        ...
        config_window.geometry("500x400")  # Made taller to accommodate both API keys
        ...
        # DeepSeek API Key
        ttk.Label(frame, text="DeepSeek API Key:", font=("Helvetica", 10, "bold")).grid(row=2, column=0, padx=5, pady=10, sticky="w")
        deepseek_key_entry = ttk.Entry(frame, width=40, textvariable=self.deepseek_tab.deepseek_api_key)
        deepseek_key_entry.grid(row=2, column=1, padx=5, pady=10, sticky="ew")
        
        ttk.Label(frame, text="Get an API key at: https://platform.deepseek.com", 
                font=("Helvetica", 8)).grid(row=3, column=0, columnspan=2, padx=5, pady=5, sticky="w")
        ...
        ttk.Button(button_frame, text="Test DeepSeek API Key", 
                  command=self.deepseek_tab.test_deepseek_key).pack(side="left", padx=5)
    ...
    def show_about(self):
        ...
        description = "A comprehensive tool for network reconnaissance, featuring Nmap scanning, Shodan intelligence, DeepSeek AI integration, and various web reconnaissance capabilities."
...
"""
    ...
    def show_deepseek_guide(self):
        """Show a window with DeepSeek usage guidelines."""
        guide_window = tk.Toplevel(self)
        guide_window.title("DeepSeek AI Usage Guide")
        guide_window.geometry("700x500")
        guide_window.configure(bg=COLORS["background"])
        ...
        # Title
        title_font = font.Font(family="Helvetica", size=14, weight="bold")
        ttk.Label(frame, text="DeepSeek AI Usage Guide", font=title_font).pack(pady=(0, 10))
...
1. Introduction to DeepSeek AI:
   - DeepSeek AI offers powerful language models for various tasks
   - Models include conversational AI (deepseek-chat) and coding assistant (deepseek-coder)
   - Perfect complement to your reconnaissance tools for analysis and insights

2. Key Features and Capabilities:
   - Natural language conversations about collected data
   - Code generation based on scan results
   - Automated analysis of reconnaissance findings
...
3. Effective Usage:
   ...
   - Use temperature slider to control creativity vs. precision
   - Set appropriate token limits for expected response length
   - Refer to specific reconnaissance data in your queries

4. Practical Examples:
   - "Analyze these Nmap scan results and identify potential vulnerabilities"
   - "Generate a Python script to parse this Shodan output"
   - "Explain the security implications of these open ports"
   - "Create a summary report of all reconnaissance findings"
   - "Suggest next steps based on these domain lookup results"

5. Integration with Other Tools:
   - Paste results from other tabs directly into DeepSeek queries
   - Use the 'Get Recon Context' function to analyze all findings
   - Extract actionable insights from complex reconnaissance data
   - Generate targeted follow-up commands for further investigation

6. API Key Management:
   - Keep your API key secure
   - Monitor token usage to manage costs
   - Use the API configuration dialog to update keys

Remember: DeepSeek AI enhances your reconnaissance capabilities but should be used alongside human expertise and judgment.
"""
...
# --- DeepSeek Tab Class ---
class DeepSeekTab(BaseTab):
    def __init__(self, notebook, parent):
        super().__init__(notebook, parent)
        
        # Set DeepSeek API key
        self.deepseek_api_key = tk.StringVar(value=DEFAULT_DEEPSEEK_API_KEY)
    ...
    def create_input_frame(self):
        """Create the input frame for DeepSeek queries."""
        input_frame = ttk.LabelFrame(self, text="DeepSeek AI Query", padding="10")
        input_frame.pack(fill="x", pady=5)
        ...
        api_key_entry = ttk.Entry(
            api_frame, width=40, textvariable=self.deepseek_api_key, show="*"
        )
        api_key_entry.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        ...
        # Query input
        ttk.Label(input_frame, text="Query:").pack(anchor="w", padx=5, pady=5)
        self.query_text = scrolledtext.ScrolledText(input_frame, wrap=tk.WORD, height=5)
        ...
        # Parameters
        ttk.Label(model_frame, text="Temperature:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.temperature_var = tk.DoubleVar(value=0.7)
        temperature_slider = ttk.Scale(model_frame, from_=0.0, to=1.0, variable=self.temperature_var, orient="horizontal")
        temperature_slider.grid(row=1, column=1, padx=5, pady=5, sticky="ew")
        ttk.Label(model_frame, textvariable=self.temperature_var).grid(row=1, column=2, padx=5, pady=5, sticky="w")
        
        ttk.Label(model_frame, text="Max Tokens:").grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.max_tokens_var = tk.IntVar(value=2048)
        max_tokens_entry = ttk.Entry(model_frame, width=10, textvariable=self.max_tokens_var)
        max_tokens_entry.grid(row=2, column=1, padx=5, pady=5, sticky="w")
        
        model_frame.columnconfigure(1, weight=1)
        
    def create_action_frame(self):
        """Create the action frame for DeepSeek controls with context-aware features."""
        self.action_frame = ttk.Frame(self, padding="5")
        self.action_frame.pack(fill="x")
        
        self.query_button = ttk.Button(
            self.action_frame, text="Send Query", style="Run.TButton", command=self.start_query
        )
        self.query_button.pack(side="left", padx=5)
        
        self.analyze_button = ttk.Button(
            self.action_frame, text="Analyze Current Data", 
            command=self.analyze_current_data
        )
        self.analyze_button.pack(side="left", padx=5)
        
        self.context_button = ttk.Button(
            self.action_frame, text="Get Recon Context", 
            command=lambda: self.query_text.insert("1.0", self.get_reconnaissance_context())
        )
        self.context_button.pack(side="left", padx=5)
        
        self.save_button = ttk.Button(
            self.action_frame, text="Save Results", style="Save.TButton", command=lambda: self.save_results("DeepSeek")
        )
        self.save_button.pack(side="right", padx=5)
        
        self.clear_button = ttk.Button(
            self.action_frame, text="Clear", command=self.clear_outputs
        )
        self.clear_button.pack(side="right", padx=5)
        
    def create_output_frame(self):
        """Create the output frame for DeepSeek results."""
        output_frame = ttk.LabelFrame(self, text="DeepSeek Response", padding="10")
        output_frame.pack(expand=True, fill="both", pady=5)
    ...
    def test_deepseek_key(self):
        ...
        # Create a thread to test the API key
        thread = threading.Thread(
            target=self.test_api_key_thread,
            args=(api_key,),
            daemon=True
        )
        thread.start()
        
    def test_api_key_thread(self, api_key):
        """Test the DeepSeek API key in a separate thread."""
        try:
            import requests
            ...
            if response.status_code == 200:
                self.after(0, lambda: messagebox.showinfo(
                    "Success", 
                    "API key is valid! Connection to DeepSeek API successful."
                ))
            else:
                error_message = response.json().get("error", {}).get("message", "Unknown error")
                self.after(0, lambda: messagebox.showerror(
                    "Error", 
                    f"Failed to validate API key. Status code: {response.status_code}\nMessage: {error_message}"
                ))
    ...
    def start_query(self):
        ...
        api_key = self.deepseek_api_key.get().strip()
        if not api_key:
            messagebox.showerror("Error", "Please enter a valid DeepSeek API key.")
            return
        ...
        # Update status and clear output
        self.queue.put(("clear", self.output_text, None))
        self.queue.put(("status", None, f"Sending query to DeepSeek API..."))
        self.query_button.config(state=tk.DISABLED)
        
        # Run in a thread
        thread = threading.Thread(
            target=self.run_query_thread,
            args=(api_key, model, query, temperature, max_tokens, self.output_text, self.query_button),
            daemon=True
        )
        thread.start()
        
    def run_query_thread(self, api_key, model, query, temperature, max_tokens, output_widget, button_to_enable):
        """Run DeepSeek API query in a thread."""
        try:
            self.queue.put(("output", output_widget, f"--- DeepSeek Query ---\n"))
            self.queue.put(("output", output_widget, f"Model: {model}\n"))
            self.queue.put(("output", output_widget, f"Temperature: {temperature}\n"))
            self.queue.put(("output", output_widget, f"Max Tokens: {max_tokens}\n"))
            self.queue.put(("output", output_widget, f"Query: {query}\n\n"))
            ...
            self.queue.put(("output", output_widget, "Sending request to DeepSeek API...\n"))
            ...
            if response.status_code == 200:
                ...
                self.queue.put(("output", output_widget, "\n--- DeepSeek Response ---\n\n"))
                self.queue.put(("output", output_widget, assistant_message + "\n"))
                
                # Show usage information if available
                if "usage" in response_data:
                    usage = response_data["usage"]
                    self.queue.put(("output", output_widget, f"\n--- Usage Info ---\n"))
                    self.queue.put(("output", output_widget, f"Prompt Tokens: {usage.get('prompt_tokens', 'N/A')}\n"))
                    self.queue.put(("output", output_widget, f"Completion Tokens: {usage.get('completion_tokens', 'N/A')}\n"))
                    self.queue.put(("output", output_widget, f"Total Tokens: {usage.get('total_tokens', 'N/A')}\n"))
                
                self.queue.put(("status", None, "DeepSeek query completed successfully."))
            else:
                error_message = response.json().get("error", {}).get("message", "Unknown error")
                self.queue.put(("output", output_widget, f"\nAPI Error (Status {response.status_code}): {error_message}\n"))
                self.queue.put(("status", None, f"DeepSeek API error: {response.status_code}"))
            
        except requests.exceptions.Timeout:
            self.queue.put(("output", output_widget, "\nRequest timed out. The API took too long to respond.\n"))
            self.queue.put(("status", None, "Error: API request timed out."))
        except requests.exceptions.RequestException as e:
            self.queue.put(("output", output_widget, f"\nHTTP Request Error: {str(e)}\n"))
            self.queue.put(("status", None, "Error during API request."))
        except Exception as e:
            self.queue.put(("output", output_widget, f"\nAn unexpected error occurred: {str(e)}\n{traceback.format_exc()}\n"))
            self.queue.put(("status", None, "Error during DeepSeek query."))
        finally:
            # Re-enable button
            if button_to_enable:
                self.after(0, lambda: button_to_enable.config(state=tk.NORMAL))
    ...
    def clear_outputs(self):
        ...
        self.queue.put(("clear", self.output_text, None))
        self.queue.put(("status", None, "Output cleared."))
    
    def get_reconnaissance_context(self):
        """Gather all current reconnaissance results for context-aware AI."""
        context = "Here's the reconnaissance data collected so far:\n\n"
        
        try:
            # Get Nmap results
            nmap_results = self.parent.nmap_tab.get_results()
            if nmap_results and len(nmap_results) > 10:
                context += "=== NMAP SCAN RESULTS ===\n"
                context += nmap_results + "\n\n"
            
            # Get Shodan results
            shodan_results = self.parent.shodan_tab.get_results()
            if shodan_results and len(shodan_results) > 10:
                context += "=== SHODAN SEARCH RESULTS ===\n"
                context += shodan_results + "\n\n"
            
            # Get Web Recon results
            for name, results in self.parent.web_recon_tab.get_all_results().items():
                if results and len(results) > 10:
                    context += f"=== {name.upper()} RESULTS ===\n"
                    context += results + "\n\n"
            
            return context
        except Exception as e:
            self.queue.put(("output", self.output_text, f"Error gathering context: {str(e)}\n"))
            return "No reconnaissance data available."

    def analyze_current_data(self):
        """Analyze all current reconnaissance data with DeepSeek AI."""
        context = self.get_reconnaissance_context()
        
        if len(context) < 50:  # Not enough data gathered yet
            self.queue.put(("output", self.output_text, "Not enough reconnaissance data available for analysis.\n"))
            self.queue.put(("output", self.output_text, "Please run some scans or searches first.\n"))
            return
        
        # Create a query that asks DeepSeek to analyze the data
        query = (
            "Analyze the following reconnaissance data and provide insights:\n"
            "1. Identify potential security vulnerabilities\n"
            "2. Highlight interesting findings\n"
            "3. Suggest next steps for investigation\n\n"
            f"{context}"
        )
        
        # Update the query text widget with our analysis request
